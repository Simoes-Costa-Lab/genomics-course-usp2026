---
title: References
parent: ATAC-seq
nav_order: 3
---

# References

This page gathers recommended datasets, tutorials, software documentation, and additional resources for learning and exploring ATAC-seq analysis.

---

# Scripts for this Module

- [ATAC Seq Markdown](/genomics-course-usp2026/assets/scripts/usp_course_atacseq.zip)

---

# Tutorials

### Harvard Bioinformatics Core ATAC-seq Workshop

Excellent hands-on introduction to ATAC-seq analysis.

* https://hbctraining.github.io/Intro-to-peak-analysis/

### ENCODE ATAC-seq Pipeline

Official ENCODE guidelines and processing workflows.

* https://www.encodeproject.org/atac-seq/

### ATAC-seq Data Analysis Tutorial

A practical overview of common ATAC-seq analysis steps.

* https://www.bioconductor.org/help/course-materials/

---

# Software Documentation

### MACS3

Peak calling software widely used for ATAC-seq and CUT&RUN.

* https://macs3-project.github.io/MACS/

### Bowtie2

Read alignment software.

* https://bowtie-bio.sourceforge.net/bowtie2

### deepTools

Visualization and quality-control toolkit.

* https://deeptools.readthedocs.io

### ChIPseeker

Peak annotation and visualization.

* https://bioconductor.org/packages/ChIPseeker

### HOMER

Motif discovery and peak annotation.

* http://homer.ucsd.edu/homer

### IGV

Genome browser for visualizing genomic data.

* https://igv.org

---

# Theory and Background

## Chromatin Accessibility

Review of chromatin organization and gene regulation.

* https://www.nature.com/articles/s41576-018-0089-8

## Open Chromatin Profiling

Original ATAC-seq publication.

Buenrostro et al., 2013

* https://doi.org/10.1038/nmeth.2688

## Understanding Genome Browsers

IGV tutorial and documentation.

* https://igv.org/doc/

---

# Recommended Review Articles

### ATAC-seq Guidelines and Best Practices

Corces et al. (2017)

Improved ATAC-seq protocol and discussion of quality metrics.

* https://doi.org/10.1038/nmeth.4396

### Chromatin Accessibility and Gene Regulation

Klemm et al. (2019)

Comprehensive review of chromatin accessibility assays and interpretation.

* https://doi.org/10.1038/s41576-018-0089-8

### The Regulatory Genome

An overview of cis-regulatory elements and enhancer biology.

* https://doi.org/10.1038/nrg3682

---

# Additional Resources

## Genome Browsers

### UCSC Genome Browser

* https://genome.ucsc.edu

### Ensembl Genome Browser

* https://www.ensembl.org

---

## Regulatory Databases

### ENCODE Project

* https://www.encodeproject.org

### SCREEN Regulatory Elements

ENCODE candidate cis-regulatory elements.

* https://screen.encodeproject.org

### GeneHancer

Integrated enhancer database.

* https://www.genecards.org/Guide/GeneHancer

---

## Motif Databases

### JASPAR

Open-access transcription factor motif database.

* https://jaspar.genereg.net

### HOCOMOCO

Curated vertebrate transcription factor motifs.

* https://hocomoco11.autosome.org

